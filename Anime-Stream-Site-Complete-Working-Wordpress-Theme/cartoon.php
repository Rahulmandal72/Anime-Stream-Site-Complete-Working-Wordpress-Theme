...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['page'])) {
    $_SESSION['cartoon_page'] = intval($_POST['page']);
    wp_safe_remote_post(get_permalink(), array('blocking' => false));
}

// Get current page
$current_page = isset($_SESSION['cartoon_page']) ? intval($_SESSION['cartoon_page']) : 1;
...
<div class="cartoon-container">
    ...
    // APPLY META QUERY IF NEEDED
    if (!empty($meta_query)) {
        $args['meta_query'] = $meta_query;
    }

    $cartoon = new WP_Query($args);

    if($cartoon->have_posts()):
        while($cartoon->have_posts()): 
            $cartoon->the_post();
    ...
    <?php 
        ...
        echo '<div class="no-results"><span>🔍</span><p>No cartoon found</p></div>';
    endif;
    ...
    <!-- PAGINATION -->
    <?php 
    $total_pages = $cartoon->max_num_pages;
    if($total_pages > 1): 
    ?>
    ...
    <div class="pagination-info">
        ...
        (Total: <strong><?php echo $cartoon->found_posts; ?></strong> cartoons)
    </div>
    <?php endif; ?>
