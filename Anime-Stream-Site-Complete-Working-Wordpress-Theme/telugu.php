...
<?php 
/* Template Name: Telugu Page */
get_header();

// Handle pagination via POST
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['page'])) {
    $_SESSION['telugu_page'] = intval($_POST['page']);
    wp_safe_remote_post(get_permalink(), array('blocking' => false));
}

// Get current page
$current_page = isset($_SESSION['telugu_page']) ? intval($_SESSION['telugu_page']) : 1;
...
<div class="telugu-container">
    ...
    $args = [
        ...
        'orderby' => 'date',
        'order' => 'DESC',
        's' => sanitize_text_field(isset($_GET['s']) ? $_GET['s'] : ''),
    ...
    // GENRE FILTER - FIX: Proper ACF field matching
    if(isset($_GET['genre']) && !empty($_GET['genre'])){
        $selected_genre = sanitize_text_field($_GET['genre']);
        $meta_query[] = [
            'key' => 'genre',
    ...
    // Add if exists with proper relation
    if(!empty($meta_query)){
    ...
    $telugu = new WP_Query($args);

    if($telugu->have_posts()):
        while($telugu->have_posts()): 
            $telugu->the_post();
    ...
    <?php 
        ...
        echo '<div class="no-results"><span>🔍</span><p>No Telugu content found</p></div>';
    endif;
    ...
    <!-- PAGINATION -->
    <?php 
    $total_pages = $telugu->max_num_pages;
    if($total_pages > 1): 
    ?>
    ...
    <div class="pagination-info">
        ...
        (Total: <strong><?php echo $telugu->found_posts; ?></strong> Telugu content)
    </div>
    <?php endif; ?>
