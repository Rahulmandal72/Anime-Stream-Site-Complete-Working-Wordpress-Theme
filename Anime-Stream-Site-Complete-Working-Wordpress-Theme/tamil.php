...
*/
<?php 
/* Template Name: Tamil Page */
get_header();

// Handle pagination via POST
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['page'])) {
    $_SESSION['tamil_page'] = intval($_POST['page']);
    wp_safe_remote_post(get_permalink(), array('blocking' => false));
}

// Get current page
$current_page = isset($_SESSION['tamil_page']) ? intval($_SESSION['tamil_page']) : 1;
...
<div class="tamil-container">
    ...
    // GENRE FROM ACF - FIX: Proper ACF field matching
    ...
    $tamil = new WP_Query($args);

    if($tamil->have_posts()):
        while($tamil->have_posts()): 
            $tamil->the_post();
    ...
    <?php 
        ...
        echo '<div class="no-results"><span>🔍</span><p>No Tamil content found</p></div>';
    endif;
    ...
    <!-- PAGINATION -->
    <?php 
    $total_pages = $tamil->max_num_pages;
    if($total_pages > 1): 
    ?>
    ...
    <div class="pagination-info">
        ...
        (Total: <strong><?php echo $tamil->found_posts; ?></strong> Tamil content)
    </div>
    <?php endif; ?>
